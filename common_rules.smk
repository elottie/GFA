# I added
# use .bim for harmonization in snp_table_chrom
...
# default:  20G, worked when I had 35 traits: 0.57G per trait
...
#    resources: mem_mb = 5120
#               partition = "largemem"
#    script: 'R/1_combine_and_format.R'

rule snp_table_chrom:
    input: 
        files = raw_data_input, 
        gwas_info = info_input,
        ref_bim = ref_path + ".bim"
    output: 
        out = temp(data_dir + "{prefix}_zmat.{chrom}.tsv")   # Use .tsv if Bash produces TSV
    params: 
        af_thresh = af_min,
        sample_size_tol = sstol_max
...
# default:  10G, worked when I had 35 traits: 0.28G per trait
rule ld_prune_plink:
    #input: zmat = data_dir + "{prefix}_zmat.{chrom}.RDS",
    input: zmat = data_dir + "{prefix}_zmat.{chrom}.tsv",
...
rule R_ldsc_full:
    ...
    input: Z = expand(data_dir + "{{prefix}}_zmat.{chrom}.tsv", chrom = range(1, 23)),
           gwas_info = info_input,
